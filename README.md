...
```bash
# Dry-run (show plan, install nothing)
curl -fsSL https://raw.githubusercontent.com/hawkyre/hawk-skills/main/install.sh | bash -s -- --dry-run

# Install one or more named skills
curl -fsSL https://raw.githubusercontent.com/hawkyre/hawk-skills/main/install.sh | bash -s -- --only code-audit --only fix-bug

# Or clone first
git clone https://github.com/hawkyre/hawk-skills.git
cd hawk-skills
./install.sh --select
```
...
| [`code-audit-hardcore`](skills/code-audit-hardcore/SKILL.md) | Whole-repo deep clean. Specialists are licensed to override repo conventions. Auto-fixes small issues; routes large ones (>5 files, schema, public API) through `/plan-small` or `/plan-large`. |
...
/code-audit-hardcore  ← deep-clean the whole repo
